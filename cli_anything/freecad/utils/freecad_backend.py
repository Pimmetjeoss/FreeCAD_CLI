...
def export_document(
    ...
    if ext in (".step", ".stp"):
        ...
        script += (
            ...
            f"    import os\n"
            f"    \n"
            f"    # Convert shapes to meshes\n"
            f"    meshes = []\n"
            f"    for obj in objs:\n"
            f"        if hasattr(obj, 'Shape'):\n"
            f"            m = MeshPart.meshFromShape(obj.Shape, LinearDeflection=0.1, AngularDeflection=0.5)\n"
            f"            meshes.append((obj.Name, m))\n"
            f"    \n"
            f"    if meshes:\n"
            f"        # Create temporary STL files for each mesh\n"
            f"        with tempfile.TemporaryDirectory() as tmpdir:\n"
            f"            stl_files = []\n"
            f"            for i, (name, mesh) in enumerate(meshes):\n"
            f"                stl_path = os.path.join(tmpdir, f'{{name}}.stl')\n"
            f"                mesh.write(stl_path)\n"
            f"                stl_files.append((name, stl_path))\n"
            f"            \n"
            f"            # Create 3MF package (basic implementation)\n"
            f"            # Note: Full 3MF support would require lib3mf or similar\n"
            f"            # This creates a simplified 3MF structure\n"
            f"            with zipfile.ZipFile({output_path!r}, 'w', zipfile.ZIP_DEFLATED) as zf:\n"
            f"                # Add content types\n"
            f"                content_types = '''<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
            ...
            f"</Types>'''\n"
            f"                zf.writestr('[Content_Types].xml', content_types)\n"
            f"                \n"
            f"                # Add relationships\n"
            f"                rels = '''<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
            f"<Relationships xmlns=\"http://schemas.openxmlformats.org/package/2006/relationships\">\n"
            f"  <Relationship Target=\"/3D/3dmodel.model\" Id=\"rel0\" Type=\"http://schemas.microsoft.com/3dmanufacturing/2013/01/3dmodel\"/>\n"
            f"</Relationships>'''\n"
            f"                zf.writestr('_rels/.rels', rels)\n"
            f"                \n"
            f"                # Create basic 3D model (simplified - would need full 3MF spec)\n"
            f"                model_content = '''<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
            f"<model unit=\"millimeter\" xmlns=\"http://schemas.microsoft.com/3dmanufacturing/core/2015/02\">\n"
            f"  <resources>\n"
            ...
            f"  </build>\n"
            f"</model>'''\n"
            f"                zf.writestr('3D/3dmodel.model', model_content)\n"
            f"                \n"
            f"                _cli_result['note'] = 'Basic 3MF structure created - full mesh data requires lib3mf integration'\n"
            f"                _cli_result['meshes_processed'] = len(meshes)\n"
        )
    else:
